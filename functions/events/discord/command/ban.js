...
if (isBAN_MEMBERS|| context.params.event.member.user.id == `${process.env.Bot_owner_id}`) {
...
*伺服器:* **${guild.name}**
*原因:* **${reason}**
*被以下管理員封鎖:* **<@${context.params.event.member.user.id}>**`,
      color: 0x0d6efd,
    },
  });
  ...
  let error = await lib.discord.channels['@0.2.0'].messages.create({
    ...
    content: `**<@${context.params.event.member.user.id}> -  您需要 BAN_MEMBERS 權限才能使用 封鎖 命令！**`,
  });
  await sleep(25000);
